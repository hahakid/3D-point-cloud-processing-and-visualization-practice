...
def load_point_list(config):
    point_list = sorted(os.listdir(os.path.join(config["root_path"], "velodyne")))
    for idx in range(len(point_list)):
        point_list[idx] = config["root_path"]+ "/" + "velodyne/" +point_list[idx]

    return point_list


def load_label_list(config):
    label_list = sorted(os.listdir(os.path.join(config["root_path"], "labels")))
    for idx in range(len(label_list)):
        label_list[idx] = config["root_path"] + "/" + "labels/" +label_list[idx]
...
def pose_init(data_root):
    calib_path = os.path.join(data_root,"calib.txt")
    calib = parse_calibration(calib_path)

    seq_pos_list = parse_poses(os.path.join(data_root, 'poses'+ ".txt"), calib)
    return seq_pos_list
...
if __name__ == "__main__":
    ...
    config = parase_config("R132.yaml")
